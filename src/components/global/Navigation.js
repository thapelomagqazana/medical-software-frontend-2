...
const Navigation = ({ isAuthenticated }) => {
    ...
    const unauthMenuItems = [
        { text: 'Home', icon: <HomeIcon />, link: '/' },
        { text: 'Login', icon: <LoginIcon />, link: '/sign-in' },
            ...
            <AppBar position="static">
                <Toolbar>
                    <Typography variant="h6" sx={{ flexGrow: 1 }}>
                        {isAuthenticated ? 'Patient Dashboard' : 'HealthHub'}
                    </Typography>
                        ...
                        {menuItems.map((item) => (
                            item.link ? (
                                <Button
                                    ...
                                    component={Link}
                                    to={item.link}
                                    sx={{ textTransform: 'none', fontWeight: location.pathname === item.link ? 'bold' : 'normal', margin: '0 8px' }}
                                >
                                    {item.text}
                                </Button>
                            ) : (
